...
def generate_post(post_type, theme, used_texts, topic, sodan_used=False):
    """投稿文を生成する（重複チェック・トピック多様化・ご相談制限あり）"""
...
- 上記の使用済み内容と全く同じ・似た内容にしない（言葉・テーマ両方）
...
def main():
    ...
    for day_offset in range(7):
        ...
        for i, time_str in enumerate(SCHEDULE):
            ...
            text = generate_post(post_type, theme, used_texts, topic, sodan_used)
            # 「ご相談ください」使用を追跡
