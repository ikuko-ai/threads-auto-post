...
def verify_post(text):
    """生成した投稿文の医学的正確性をチェックする。問題があればFalseを返す"""
    client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)

    verify_prompt = f"""あなたは歯科医師です。以下のSNS投稿文を医学的観点から厳しくチェックしてください。

【投稿文】
{text}

以下の観点でチェックしてください：
1. 医学的・歯科的に誤った情報や根拠のない主張が含まれていないか
2. 複数のテーマが混在して文章として矛盾・不整合が生じていないか
3. 患者に誤解を与えるような誇張・不正確な表現がないか
4. 「就寝時も装着できる」「手入れが簡単」など事実と異なる可能性のある記述がないか

判定：問題なければ「OK」、問題があれば「NG: （理由を簡潔に）」とだけ答えてください。"""
...
{nsen_instruction}{sodan_instruction}- 本文のみ出力（説明不要）"""
    ...
    for attempt in range(3):
        ...
        print(f"  再生成します（試行{attempt + 2}回目）")

    # 3回NGの場合はそのまま使用（承認列に警告を入れる）
    print("  警告：3回チェックNGのため、要確認フラグを立てます")
    return add_line_breaks(text + "\n※要確認")
