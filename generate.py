...
# 投稿スケジュール（JST時刻）
# 1日21件→12件に削減（朝3・昼3・夕2・夜4）。
# GitHub Actionsのcron窓 JST 7:00-8:55 / 12:00-13:55 / 17:00-18:55 / 20:00-21:55 に全て収まる。
SCHEDULE = [
    "07:00", "07:30", "08:00",
    "12:00", "12:30", "13:00",
    "17:00", "18:00",
...
TOPICS_DATA = {
    ...
    "健康な歯の価値": [
        ...
        ("教育", "歯は削るほど寿命が縮んでいくという考え方"),
        ("教育", "天然の歯に代わる人工物はまだ無いと言われる理由"),
        ("教育", "歯を1本失うと隣の歯や噛み合わせにも負担がかかる"),
        ("教育", "生涯にかかる歯の費用は失うほど積み上がりやすい"),
        ("教育", "毎日3食使い続ける歯は体の中でも働き者な部分"),
        ("共感", "バッグや時計には使えても歯には後回しになってしまう"),
        ("共感", "歯の治療費を高いと感じてきた方へ"),
        ("事例", "歯を抜かずに残す選択をして良かったと話された方"),
        ("人柄", "私が1本の歯を簡単に抜かないと決めている理由"),
        ("導線", "今ある歯をこれから何年使いたいか考えてみませんか"),
    ],
    "一回の治療を長持ちさせる": [
...
# これにより似た投稿しか作れないテーマが毎日出るのを防ぐ。
# 1日21件→12件への削減（週147→84）に伴い、各上限は旧値×0.6（四捨五入・最低1）に縮小。
# 2026-08-09：テーマの偏りを是正。健康寿命4→8・健康な歯の価値2→6に増やし、
# その分を治療系の大口テーマ（精密な入れ歯・歯周病・セラミックほか）から差し引いた。
# 合計は週84枠ちょうど。以前は合計86でpool[:84]の切り捨てが起き、
# TOPICS_DATAの末尾テーマ（口腔崩壊）だけが上限より2件少なくなっていたため揃えた。
THEME_WEEKLY_CAP = {
    "精密な入れ歯": 9,
    "歯周病": 8,
    "セラミック": 7,
...
{nsen_instruction}{sodan_instruction}"""
    ...
    # 全滅時：最も短い候補を採用するが、承認列で「スキップ」扱いにできるようNGを知らせる
    # （本文への「※要確認」付与は廃止：付与すると承認列がOKのままの行がそのまま投稿されてしまうため）
    print("  警告：チェックNGのため、この行は承認列に「スキップ」を設定します")
    return (shortest or text), False
...
def main():
    ...
    for day_offset in range(7):
        ...
        for i, time_str in enumerate(SCHEDULE):
            ...
            text, ok = generate_post(post_type, theme, all_used_texts, topic, sodan_used,
            ...
            # チェック全滅（ok=False）の行は承認列に「スキップ」を書き込み、
            # post.py側の投稿対象（E列が「スキップ」「投稿済」以外）から確実に除外する
            approval = "OK" if ok else "スキップ"
            day_rows.append([date_str, time_str, post_type, text, approval, ""])
