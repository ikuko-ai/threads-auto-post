...
def select_day_topics(n=21):
    """1日分のトピックをカテゴリが偏らないよう選択する"""
    categories = list(TOPICS_BY_CATEGORY.keys())
    selected = []
    used_categories = []

    # まず各カテゴリから1つずつ選ぶ（7カテゴリ×1=7個）
    shuffled_cats = categories.copy()
    random.shuffle(shuffled_cats)
    for cat in shuffled_cats:
        topics = TOPICS_BY_CATEGORY[cat].copy()
        random.shuffle(topics)
        selected.append((cat, topics[0]))
        used_categories.append(cat)

    # 残りはカテゴリをラウンドロビンで追加（同カテゴリが連続しないよう）
    remaining_pool = []
    for cat in categories:
        for topic in TOPICS_BY_CATEGORY[cat]:
            if topic not in [t for _, t in selected]:
                remaining_pool.append((cat, topic))
    random.shuffle(remaining_pool)

    # 同じカテゴリが連続しないよう並べ替えながら追加
    while len(selected) < n and remaining_pool:
        last_cat = selected[-1][0] if selected else None
        # 直前と異なるカテゴリを優先
        for i, (cat, topic) in enumerate(remaining_pool):
            if cat != last_cat:
    ...
    random.shuffle(selected)  # 時間帯順にランダム配置
    return [topic for _, topic in selected[:n]]
